...
namespace Web_ECommerce.Controllers
{
    public class CompraUsuarioController : HelpQrCode
    {
        ...
        public CompraUsuarioController(UserManager<Cliente> userManager, InterfaceCompraUsuarioApp InterfaceCompraUsuarioApp, IWebHostEnvironment environment)
        {
        ...
        public async Task<JsonResult> AdicionarProdutoCarrinho(string id, string nome, string qtd)
        {
            ...
            if (usuario != null)
            {
                await _InterfaceCompraUsuarioApp.Add(new Alocacao
                {
                    IdProduto = Convert.ToInt32(id),
                    QtdCompra = Convert.ToInt32(qtd),
                    Estado = GrupoEnum.Produto_Carrinho,
                    UserId = usuario.Id
                });
