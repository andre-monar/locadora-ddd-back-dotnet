...
{
    public class RepositoryAlocacao : RepositoryGenerics<Alocacao>, IAlocacao
    {

        private readonly DbContextOptions<ContextBase> _optionsbuilder;

        public RepositoryAlocacao()
        {
        ...
        public async Task<bool> ConfirmaCompraCarrinhoUsuario(string userId)
        {
            try
            {
                ...
                {
                    var compraUsuario = new Alocacao();
                    compraUsuario.ListaProdutos = new List<Carro>();
                    ...
                    {
                        p.Estado = GrupoEnum.Produto_Comprado;
                    });
        ...
        public async Task<Alocacao> ProdutosCompradosPorEstado(string userId, GrupoEnum estado)
        {
            using (var banco = new ContextBase(_optionsbuilder))
            {
                var compraUsuario = new Alocacao();
        ...
        public async Task<int> QuantidadeProdutoCarrinhoUsuario(string userId)
        {
            using (var banco = new ContextBase(_optionsbuilder))
            {
                return await banco.CompraUsuario.CountAsync(c => c.UserId.Equals(userId) && c.Estado == GrupoEnum.Produto_Carrinho);
            }
        }
    }
}
